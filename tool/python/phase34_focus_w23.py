...
def attack():
    ...
    for g in range(29):
        ...
        # Check for fragments
        found = [w for w in ["PENUMBRA", "SHADOW", "PIGEON", "LITTLE", "WHILE", "SAID", "PATH", "WAY", "AGO"] if w in text]
        if found:
            print(f"G={g:2d} | Found: {found}")
            if "PIGEON" in found or len(found) >= 2:
