...
CRIBS = [
    "ALITTLE", "WHILE", "AGO", "YOUWERE", "WALKING", "STANDING", "SITTING",
    "DEPEND", "PENUMBRA", "SHADOW", "OSHADOW", "SAID", "PIGEON", "PATH", "WAY"
]
...
def attack():
    ...
    for w in range(2, 41):
        ...
        # Pattern: Standard Boustrophedon
        stream = []
        ...
        for g in range(29):
            ...
            if len(found) >= 2:
                # Prioritize hits with PIGEON or quote-heavy hits
                score = len(found)
                if "PIGEON" in found: score += 5
                if "ALITTLE" in found: score += 5

                if score >= 7:
                    print(f"W={w} G={g:2d} | Score={score} | Found: {found}")
