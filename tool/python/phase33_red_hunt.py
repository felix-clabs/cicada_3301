...
print(f"Red runes: {len(red)}")

for g in range(29):
    dec = [(red[i] - P71_KEY[i % len(P71_KEY)] - g) % 29 for i in range(len(red))]
    matches = sum(1 for i in range(min(len(dec), len(target))) if dec[i] == target[i])
    if matches >= 5:
        latin = "".join([rt.RUNE_TO_LATIN[rt.INDEX_TO_RUNE[idx]].split('/')[0] for idx in dec])
        print(f"G={g:2d} Matches={matches:2d} | {latin}")

# Also try without key (Bypass)
print("\n--- Bypass Scan ---")
for g in range(29):
